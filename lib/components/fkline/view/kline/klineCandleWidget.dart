...
class _CandlePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < (listData ?? []).length; i++) {
      ...
      candlestickTop = height -
          ((market?.open ?? 0) - (priceMin ?? 0)) * heightPriceOffset +
          topMargin;
      candlestickBottom = height -
          ((market?.close ?? 0) - (priceMin ?? 0)) * heightPriceOffset +
          topMargin;
      ...
      // 绘制上影线/下影线
      double low = height -
          ((market?.low ?? 0) - (priceMin ?? 0)) * heightPriceOffset +
          topMargin;
      double high = height -
          ((market?.high ?? 0) - (priceMin ?? 0)) * heightPriceOffset +
          topMargin;
      double candlestickCenterX =
          candlestickLeft + (candlestickWidth ?? 0).ceilToDouble() / 2.0;
      double closeOffsetY = height -
          ((market?.close ?? 0) - (priceMin ?? 0)) * heightPriceOffset +
          topMargin;
      ...
      if (market?.high == pMax && !maxPricePainted) {
        ...
        _drawText(
            canvas,
            Offset(textOrginX, high - kCandleTextHight / 2),
            (market?.high ?? 0).toStringAsPrecision(kGridPricePrecision),
            isLeft);
      ...
      if (market?.low == pMin && !minPricePainted) {
        ...
        _drawText(
            canvas,
            Offset(textOrginX, low - kCandleTextHight / 2),
            (market?.low ?? 0).toStringAsPrecision(kGridPricePrecision),
            isLeft);
