  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder(
      ...
      builder: (BuildContext context, AsyncSnapshot<Market> snapshot) {
        ...
        return market == null
            ? Container()
            : market.isShowCandleInfo ?? false
                ? CustomPaint(
                    size: Size(bloc.screenWidth, (market.gridTotalHeight ?? 0)),
                    painter: _KlineCandleCrossPainter(
                        market, (bloc.candlestickWidth ?? 0)),
                  )
                : Container();
      },
    );
...
class _KlineCandleCrossPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    Paint paintV = Paint()
      ...
      ..strokeWidth = crossVLineWidth ?? 0
      ..isAntiAlias = true;

    // 画竖线
    canvas.drawLine(Offset(market?.offset?.dx ?? 0, originY),
        Offset(market?.offset?.dx ?? 0, (market?.gridTotalHeight ?? 0) + originY), paintV);
    // 画点
    Paint pointPaint = Paint()..color = crossPointColor;
    Offset realOffset = Offset(market?.offset?.dx ?? 0, (market?.offset?.dy ?? 0) + originY);
    ...
    double rectTopY = originY + (market?.offset?.dy ?? 0) - halfRectHeight;
    double rectBottomY = originY + (market?.offset?.dy ?? 0) + halfRectHeight;

    if ((market?.offset?.dx ?? 0) < size.width / 2) {
      ...
      offset3 = Offset(crossHlineStartOriginX, originY + (market?.offset?.dy ?? 0));
      ...
      offset3 = Offset(crossHlineEndOriginX, originY + (market?.offset?.dy ?? 0));
      offset4 = Offset(crossHlineEndOriginX + halfRectHeight, rectBottomY);
      offset5 = Offset(size.width - hMargin, rectBottomY);
    }

    // 画横线
    canvas.drawLine(Offset(crossHlineStartOriginX, (market?.offset?.dy ?? 0) + originY),
        Offset(crossHlineEndOriginX, (market?.offset?.dy ?? 0) + originY), paintH);
    List<Offset> points = [
      offset1,
      offset2,
      offset3,
    ...
    // 绘制文字
    Offset of = Offset(
        closeTextOriginX, originY + (market?.offset?.dy ?? 0) - closePainter.height / 2);
