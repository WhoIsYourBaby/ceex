...
// 财富
class AssetsManageMoneyWidget extends StatefulWidget {
  final AssetsCubit cubit;
  const AssetsManageMoneyWidget({required this.cubit, super.key});

  @override
  State<AssetsManageMoneyWidget> createState() =>
      _AssetsManageMoneyWidgetState();
}

class _AssetsManageMoneyWidgetState extends State<AssetsManageMoneyWidget>
    ...
    return CustomScrollView(slivers: [
        ...
        child: Row(crossAxisAlignment: CrossAxisAlignment.end, children: [
          ...
          Text(
            cubit.state.showAssets ?? false
                ? "${cubit.state.totalType == TotalType.btcValue ? cubit.state.btcValue : (cubit.state.contractMoney ?? "0")}"
                : "******",
            style: appTheme.colorSet.textBlack
          ...
          Padding(
              ...
              child: Text(
                cubit.state.totalType == TotalType.btcValue ? "BTC" : "USDT",
                style: appTheme.colorSet.textBlack
                    .style(fontSize: 12, fontWeight: FontWeight.w600),
              )),
      ...
      SliverToBoxAdapter(
        ...
        children: [
          ...
          Text(
            cubit.state.showAssets ?? false
                ? ("≈¥ ${NumberUtils.getCountConn(NumberUtils.mulString((cubit.state.rates ?? 0).toString(), (cubit.state.contractMoney ?? 0).toString()), 2)}")
                : "***",
            style: appTheme.colorSet.textBlack
                .style(fontSize: 12, fontWeight: FontWeight.w600),
          ),
      ...
      //   itemBuilder: (context, index) => EmptyWidget(),
      // ): EmptyWidget()),
    ]);
  }
