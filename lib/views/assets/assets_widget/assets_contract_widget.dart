...
// 合约
class AssetsContractWidget extends StatefulWidget {
  final AssetsCubit cubit;
  const AssetsContractWidget({required this.cubit, super.key});

  @override
  State<AssetsContractWidget> createState() => _AssetsContractWidgetState();
}

class _AssetsContractWidgetState extends State<AssetsContractWidget>
    ...
    return CustomScrollView(slivers: [
                ...
                children: [
                  ...
                  Text(
                    cubit.state.showAssets ?? false
                        ? "${cubit.state.totalType == TotalType.btcValue ? cubit.state.btcValue : (cubit.state.financingtMoney ?? "0")}"
                        : "******",
                    style: appTheme.colorSet.textBlack
      ...
      SliverToBoxAdapter(
        ...
        children: [
          ...
          Text(
            cubit.state.showAssets ?? false
                ? ("≈¥ ${NumberUtils.getCountConn(NumberUtils.mulString((cubit.state.rates ?? 0).toString(), (cubit.state.financingtMoney ?? 0).toString()), 2)}")
                : "***",
            style: appTheme.colorSet.textBlack
                .style(fontSize: 12, fontWeight: FontWeight.w600),
          ),
      ...
      SliverToBoxAdapter(
          child: (cubit.state.connectMoneyList ?? []).length > 0 ? ListView.builder(
        shrinkWrap: true,
        itemCount: (cubit.state.connectMoneyList ?? []).length,
  ...
  Widget itemWidget(context, index) {
    final token = cubit.state.connectMoneyList![index];
    if (!(cubit.state.showZero ?? true) &&
      ...
      // );
       return LevelViewItem(
        tokenName: token.tokenName ?? "--",
        total: cubit.state.showAssets ?? false ? NumberUtils.getCountConn(token.locked ?? "0",4) : "***",
        showAssets: cubit.state.showAssets ?? false,
        balanceUpdatedAt: NumberUtils.getCountConn(token.total ?? "0",4),
        iconUrl: token.iconUrl,
        rates: cubit.state.rates ?? 0,
        free: NumberUtils.getCountConn(token.total ?? "0",4),
        showFree: true,
        isLevel: false,
        onTap: () {
          Navigator.push(context, TokenDetailView.route(token.tokenId ?? ""));
