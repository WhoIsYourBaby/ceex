...
class EChatView extends StatelessWidget {
  final ChartType chartType;
  final List<EChartPieBean> dataList;
  EChatView({Key? key, this.chartType = ChartType.PIE, required this.dataList})
      : super(key: key);
...
class _PieChatState extends State<PieChatView> with TickerProviderStateMixin {
  //来个动画控制器
...
//计算半径
      ...
      for (var i = 0; i < list.length; i++) {
        ...
        double realNumber = entity.number;
        ...
        if (isDrawLine) {
          _drawLineAndText(canvas, startRadin + sweepRadin / 2, sweepRadin,
              tagRadius, entity.title, entity.color, unitNumber, realNumber);
        }
...
//返回true 刷新
  ...
  List<Map<String, double>> usedPoints = [];
  ...
  void _drawLineAndText(
    ...
    // 判断是否需要调整坐标
    for (var point in usedPoints) {
      if ((point['startX'] == startX && point['startY'] == startY) &&
          (point['stopX'] == stopX && point['stopY'] == stopY)) {
        stopY = stopY + 15;
      }
    }
    ...
    for (var point in usedPoints) {
      if (((point['startX'] ?? 0) - startX).abs() < 20) {
        stopY += 20 * (endX - startX).sign;
      }
    }
